...
const EVENT_IDS: Dictionary = {
	...
	&"political_unrest": &"chain_political_unrest",
}
...
static func get_warning(chain_id: StringName) -> Dictionary:
	match chain_id:
		...
		&"political_unrest":
			return {
				"title": "Шёпот в зале совета",
				"body": "Стража перехватила письма вельмож, обсуждающих слабость короны. Через три дня заговорщики решат, готовы ли выступить открыто.",
				"choices": [
					_choice(&"buy_loyalty", "Купить верность знати", "Потратить 8 золота. Быстро снизит напряжение. Риск: требования повторятся.", {"gold": 8}, {"gold": -8, "stability": 2}, "Сундуки отправлены влиятельным семьям. На время их голоса стали тише."),
					_choice(&"public_inquiry", "Начать открытое расследование", "Потратить 4 золота. Может вернуть доверие, но поможет виновным скрыться. Риск: умеренный.", {"gold": 4}, {"gold": -4, "stability": 1}, "Глашатаи объявили расследование, а судьи начали собирать показания."),
					_choice(&"ignore_unrest", "Не отвечать на слухи", "Не тратить ресурсы. Высокий риск: молчание примут за слабость. Автоматически через три дня.", {}, {}, "Корона не ответила на предупреждения."),
				],
			}
	return {}
...
static func _branch(chain_id: StringName, choice_id: StringName) -> Dictionary:
	var branches: Dictionary = {
		...
		&"political_unrest": {
			&"buy_loyalty": _outcome("Цена верности растёт", "Получившие золото семьи требуют новых должностей. Остальные видят, что преданность продаётся.", "Заговор отложен", "Большая часть знати отошла от заговора, но казна создала опасный обычай покупать тишину.", {"stability": 1}),
			&"public_inquiry": _outcome("Свидетели называют имена", "Горожане приносят судьям письма и печати. Несколько заговорщиков покинули столицу.", "Совет очищен", "Открытый суд раскрыл сеть заговорщиков. Часть виновных скрылась, но доверие к короне выросло.", {"stability": 4}),
			&"ignore_unrest": _outcome("Знать собирает дружины", "Молчание убедило заговорщиков, что двор не знает об их планах. В поместьях раздают оружие.", "Мятеж у ворот столицы", "Стража остановила выступление, но кварталы были разграблены, а власть короны ослабла.", {"stability": -8}),
		},
	}
